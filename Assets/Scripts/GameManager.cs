...
public class GameManager : MonoBehaviour
{
    ...
    public enum GamePhase { Menu, Story, Call, Intro, Quiz, Bandage, CPR, AED, Burn, Choking, Heatstroke, Triage, Stabilize, Result, Shop, ElectricShock, Poisoning, EmergencyKit, DogPoisoning }
    public enum VictimType { BikeAccident, BleedingWound, UnconsciousPerson, BurnInjury, Choking, Heatstroke, TriageScene, ElectricShock, Poisoning, ParkCleanup, BoneFracture, AllergicShock, DrowningVictim, DiabeticShock, PanicAttack, Stroke, DogPoisoning, HeartAttack, Snakebite }
    ...
    public class FirstAidMission
    {
        ...
        [System.NonSerialized] public bool hasGaffers;
    }
    ...
    public bool boneFractureHelped = false;
    public bool allergicShockHelped = false;
    public bool parkCleanupHelped = false;
    public bool drowningHelped = false;
    public bool diabeticShockHelped = false;
    public bool panicAttackHelped = false;
    public bool strokeHelped = false;
    public bool dogPoisoningHelped = false;
    public bool heartAttackHelped = false;
    public bool snakebiteHelped = false;
    ...
    public void TryToggleMonitor()
    {
        if (monitorManager == null && monitorPanel != null)
            monitorManager = monitorPanel.GetComponent<MonitorManager>();

        if (monitorManager == null || !CanOpenMonitor())
            return;

        monitorManager.ToggleMonitor();
    }

    public bool CanOpenMonitor()
    {
        switch (currentPhase)
        {
            case GamePhase.Menu:
        ...
        foreach (var m in availableMissions)
        {
            switch (m.type)
            {
                ...
                case VictimType.ParkCleanup:
                    ...
                    break;
                case VictimType.BoneFracture:
                    m.hasPlayed = boneFractureHelped;
                    break;
                case VictimType.AllergicShock:
                    m.hasPlayed = allergicShockHelped;
                    break;
                case VictimType.DrowningVictim:
                    m.hasPlayed = drowningHelped;
                    break;
                case VictimType.DiabeticShock:
                    m.hasPlayed = diabeticShockHelped;
                    break;
                case VictimType.PanicAttack:
                    m.hasPlayed = panicAttackHelped;
                    break;
                case VictimType.Stroke:
                    m.hasPlayed = strokeHelped;
                    break;
                case VictimType.DogPoisoning:
                    m.hasPlayed = dogPoisoningHelped;
                    break;
                case VictimType.HeartAttack:
                    m.hasPlayed = heartAttackHelped;
                    break;
                case VictimType.Snakebite:
                    m.hasPlayed = snakebiteHelped;
                    break;
            }
        }
    }
    ...
    public void CleanUpActiveInstances()
    {
        foreach (var m in availableMissions)
        {
            ...
            switch (m.type)
            {
                ...
                case VictimType.BoneFracture: helped = boneFractureHelped; break;
                case VictimType.AllergicShock: helped = allergicShockHelped; break;
                case VictimType.DrowningVictim: helped = drowningHelped; break;
                case VictimType.DiabeticShock: helped = diabeticShockHelped; break;
                case VictimType.PanicAttack: helped = panicAttackHelped; break;
                case VictimType.Stroke: helped = strokeHelped; break;
                case VictimType.DogPoisoning: helped = dogPoisoningHelped; break;
                case VictimType.HeartAttack: helped = heartAttackHelped; break;
                case VictimType.Snakebite: helped = snakebiteHelped; break;
            }
    ...
    public void StartIntroPhase()
    {
        ...
        // Restore camera zoom and target player
        InitializeOrthoSize();
        if (Camera.main != null)
        {
            Camera.main.orthographicSize = originalOrthographicSize;
    ...
    public void TriggerAccident(VictimType type)
    {
        ...
        else if (type == VictimType.BoneFracture)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.AllergicShock)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.DrowningVictim)
        {
            currentPhase = GamePhase.CPR;  // Reuse CPR phase for drowning rescue
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.DiabeticShock)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.PanicAttack)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.Stroke)
        {
            StartCoroutine(TransitionToStroke(type));
        }
        else if (type == VictimType.DogPoisoning)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.HeartAttack)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
        else if (type == VictimType.Snakebite)
        {
            StartCoroutine(TransitionToNewMission(type));
        }
    }

    IEnumerator TransitionToStroke(VictimType type)
    {
        yield return new WaitForSeconds(0.8f);
        if (introPanel != null) introPanel.SetActive(false);
        if (StrokeManager.Instance != null) StrokeManager.Instance.Activate();
    }

    IEnumerator TransitionToNewMission(VictimType type)
    {
        yield return new WaitForSeconds(0.8f);
        if (introPanel != null) introPanel.SetActive(false);
        StartNewMissionPanel(type);
    }

    public void StartNewMissionPanel(VictimType type)
    {
        GameBootstrap bs = FindFirstObjectByType<GameBootstrap>();
        if (bs != null) bs.ShowNewMissionPanel(type);
    }

    public void CompleteNewMission(VictimType type, int coins)
    {
        switch (type)
        {
            case VictimType.BoneFracture:  boneFractureHelped  = true; break;
            case VictimType.AllergicShock: allergicShockHelped = true; break;
            case VictimType.DrowningVictim: drowningHelped     = true; break;
            case VictimType.DiabeticShock: diabeticShockHelped = true; break;
            case VictimType.PanicAttack:   panicAttackHelped   = true; break;
            case VictimType.Stroke:        strokeHelped        = true; break;
            case VictimType.DogPoisoning:  dogPoisoningHelped  = true; break;
            case VictimType.HeartAttack:   heartAttackHelped   = true; break;
            case VictimType.Snakebite:     snakebiteHelped     = true; break;
        }
        ...
        ShowMissionReport(type.ToString(), true, () =>
        {
            if (ambulanceManager != null)
            {
                ambulanceManager.gameObject.SetActive(true);
                ambulanceManager.SpawnAmbulance(new Vector2(6, 2), CheckAllHelped);
            }
            else CheckAllHelped();
        ...
        if (currentPhase == GamePhase.Intro)
        {
            ...
            if (activeCount == 0 && availableMissions.Count > 0)
            {
                ...
                if (matchingMissions.Count > 0)
                {
                    ...
                    GameBootstrap bootstrap = FindFirstObjectByType<GameBootstrap>();
                    if (bootstrap != null && AudioManager.Instance != null && bootstrap.telephoneSound != null)
                    {
                        AudioManager.Instance.PlaySFXWithDuration(bootstrap.telephoneSound, 1.5f);
                    }
                    
                    if (missionBannerManager != null)
                    {
                        missionBannerManager.ShowBanner(0, "Neuer Notfall: " + chosen.title);
                    }
    ...
    private IEnumerator RandomMissionRoutine()
    {
        while (true)
        {
            yield return new WaitForSeconds(Random.Range(5f, 10f)); // Checks every 5-10 seconds
            SpawnRandomMissionIfNeeded();
        }
    }
    ...
    public void StartMission(string missionId)
    {
        ...
        // Onlookers/gaffers are active for all missions
        mission.hasGaffers = true;
        Debug.Log("Mission spawned: " + missionId + " | hasGaffers: " + mission.hasGaffers);
    ...
    public void ShowMissionReport(string missionName, bool success, System.Action onConfirm)
    {
        ...
        float timeTaken = missionStartTime > 0 ? (Time.time - missionStartTime) : 0f;
        int timeBonus = 0;

        if (success && timeTaken > 0)
        {
            if (timeTaken < 30f) timeBonus = 50;
            else if (timeTaken < 60f) timeBonus = 25;
            else if (timeTaken < 120f) timeBonus = 10;
            
            // Save time to HighscoreManager
            if (HighscoreManager.Instance != null && System.Enum.TryParse(missionName, out VictimType victimType))
            {
                HighscoreManager.Instance.SaveTime(victimType, timeTaken);
            }
        }     // 2. Base coin reward
        int rewardCoins = 15;
        if (success)
        {
            ...
            timeBonus = Mathf.Max(0, 100 - Mathf.FloorToInt(timeTaken));
            rewardCoins += timeBonus;
        }
        else
        {
            rewardCoins = 5;
        ...
        GameObject reportObj = UIFactory.CreateUIElement(canvasRT, "MissionReportDialog", Vector2.zero, new Vector2(760, 800));
        ...
        // ── Drop shadow (parented to reportObj so it drags with it) ──
        GameObject shadow = UIFactory.CreateUIElement(reportRT, "ReportShadow", new Vector2(5, -5), new Vector2(760, 800));
        ...
        GameObject header = UIFactory.CreateUIElement(reportRT, "Header", new Vector2(0, 381), new Vector2(754, 38));
        UIFactory.SetupImage(header, winHeaderSprite, false);
        header.AddComponent<WindowDragger>();

        // ── Accent Line ──
        GameObject accentLine = UIFactory.CreateUIElement(reportRT, "AccentLine", new Vector2(0, 360), new Vector2(754, 4));
        ...
        string reportTitle = loc != null ? "📊 " + loc.Get("report_success").Replace("!","") : "📊 Einsatz-Bericht";
        Text headerTxt = UIFactory.CreateText(header.transform, "Title", reportTitle, new Vector2(10, 0), 22, TextAnchor.MiddleLeft);
        headerTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(720, 38);
        headerTxt.color = Color.white;

        // ── Status Badge (large centered) ──
        string statusEmoji = success ? "🏅" : "❌";
        string statusLabel = success 
            ? (loc != null ? loc.Get("report_success") : "MISSION ERFOLGREICH!") 
            : (loc != null ? loc.Get("report_fail") : "MISSION FEHLGESCHLAGEN!");
        ...
        GameObject badgeBg = UIFactory.CreateUIElement(reportRT, "BadgeBg", new Vector2(0, 305), new Vector2(720, 55));
        badgeBg.GetComponent<Image>().color = badgeBgColor;

        Text statusTxt = UIFactory.CreateText(reportRT, "StatusText", $"{statusEmoji}  {statusLabel}", new Vector2(0, 305), 26, TextAnchor.MiddleCenter);
        statusTxt.color = statusColor;
        statusTxt.fontStyle = FontStyle.Bold;
        statusTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(720, 55);

        // ── Mission Name ──
        Text mNameTxt = UIFactory.CreateText(reportRT, "MissionName", $"Mission: {missionName}", new Vector2(0, 260), 18, TextAnchor.MiddleCenter);
        mNameTxt.color = new Color(0.3f, 0.3f, 0.35f);
        mNameTxt.fontStyle = FontStyle.Normal;
        mNameTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(720, 26);
        ...
        float spacing = 16f;
        float startX = -(cardWidth * 1.5f) - (spacing * 1.5f);

        // Card 1: Time Taken
        string timeStr = timeTaken > 0 ? $"{(int)timeTaken}s" : "N/A";
        CreateStatCard(reportRT, "TimeCard", "⏱️ ZEIT", timeStr, 
            new Vector2(startX, cardY), new Vector2(cardWidth, cardHeight),
            new Color(0.2f, 0.5f, 0.8f), new Color(0.9f, 0.95f, 1f));

        // Card 2: Richtige Aktionen
        string correctLabel = loc != null ? "✓ " + loc.Get("report_success").Substring(0, Mathf.Min(7, loc.Get("report_success").Length)) : "✓ RICHTIG";
        ...
        CreateStatCard(reportRT, "AccuracyCard", "◎ GENAUIGKEIT", accText,
            new Vector2(startX + 3 * (cardWidth + spacing), cardY), new Vector2(cardWidth, cardHeight),
            accColor, accBgColor);

        // ── Accuracy Progress Bar ──
        float barY = 140f;
        GameObject barBg = UIFactory.CreateUIElement(reportRT, "AccBarBg", new Vector2(0, barY), new Vector2(700, 14));
        barBg.GetComponent<Image>().color = new Color(0.82f, 0.82f, 0.85f);

        float barFillWidth = Mathf.Max(6, 700f * (accPercent / 100f));
        GameObject barFill = UIFactory.CreateUIElement(reportRT, "AccBarFill", 
            new Vector2(-(700f - barFillWidth) / 2f, barY), new Vector2(barFillWidth, 14));
        ...
        // Bar label
        string barLabelStr = loc != null ? loc.Get("report_coins_earned") + $"{accPercent:F0}%" : $"Erfolgsquote: {accPercent:F0}%";
        Text barLabel = UIFactory.CreateText(reportRT, "AccLabel", $"Erfolgsquote: {accPercent:F0}%", new Vector2(0, barY - 16f), 14, TextAnchor.MiddleCenter);
        barLabel.color = new Color(0.4f, 0.4f, 0.45f);
        barLabel.fontStyle = FontStyle.Normal;
        barLabel.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 20);
        Shadow barShadow = barLabel.GetComponent<Shadow>();
        if (barShadow != null) barShadow.effectColor = Color.clear;

        // ── Reward Section ──
        float rewardY = 90f;
        GameObject rewardBg = UIFactory.CreateUIElement(reportRT, "RewardBg", new Vector2(0, rewardY), new Vector2(700, 40));
        rewardBg.GetComponent<Image>().color = new Color(0.92f, 0.9f, 0.82f);

        string rewardStr = loc != null ? "💰 " + loc.Get("report_coins_earned") + $"+{rewardCoins}" : $"💰 Belohnung: +{rewardCoins} Münzen (Inkl. {timeBonus} Zeitbonus)";
        Text rewardTxt = UIFactory.CreateText(reportRT, "RewardText", $"💰 Belohnung: +{rewardCoins} Münzen (Inkl. {timeBonus} Zeitbonus)", new Vector2(0, rewardY), 18, TextAnchor.MiddleCenter);
        rewardTxt.color = new Color(0.55f, 0.4f, 0.05f);
        rewardTxt.fontStyle = FontStyle.Bold;
        rewardTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(680, 40);
        ...
        Text medalTxt = UIFactory.CreateText(reportRT, "MedalText", medalText, new Vector2(0, 55f), 16, TextAnchor.MiddleCenter);
        medalTxt.color = new Color(0.35f, 0.35f, 0.4f);
        medalTxt.fontStyle = FontStyle.Italic;
        medalTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 26);
        Shadow medalShadow = medalTxt.GetComponent<Shadow>();
        if (medalShadow != null) medalShadow.effectColor = Color.clear;

        // ═══════════════════════════════════════════════════════════════
        // ── EDUCATIONAL DEBRIEF SECTION: "Was habe ich gelernt?" ──
        // ═══════════════════════════════════════════════════════════════
        string debriefKey = GetDebriefKey(missionName);
        if (!string.IsNullOrEmpty(debriefKey))
        {
            float debriefY = 20f;

            // Debrief header
            string debriefHeader = loc != null ? loc.Get("debrief_header") : "📚 Was habe ich gelernt?";
            GameObject debriefHeaderBg = UIFactory.CreateUIElement(reportRT, "DebriefHeaderBg", new Vector2(0, debriefY), new Vector2(720, 30));
            debriefHeaderBg.GetComponent<Image>().color = new Color(0.15f, 0.35f, 0.6f);
            Text debriefHeaderTxt = UIFactory.CreateText(reportRT, "DebriefHeader", debriefHeader, new Vector2(0, debriefY), 16, TextAnchor.MiddleCenter);
            debriefHeaderTxt.color = Color.white;
            debriefHeaderTxt.fontStyle = FontStyle.Bold;
            debriefHeaderTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 30);

            // Steps (3 correct first-aid steps)
            float stepY = debriefY - 30f;
            string stepsLabel = loc != null ? loc.Get("debrief_steps_label") : "Richtige Erste-Hilfe-Schritte:";
            Text stepsLabelTxt = UIFactory.CreateText(reportRT, "StepsLabel", stepsLabel, new Vector2(-15, stepY), 13, TextAnchor.MiddleLeft);
            stepsLabelTxt.color = new Color(0.1f, 0.5f, 0.2f);
            stepsLabelTxt.fontStyle = FontStyle.Bold;
            stepsLabelTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 20);

            for (int s = 1; s <= 3; s++)
            {
                string step = loc != null ? loc.Get(debriefKey + "_hb_step" + s) : "";
                if (!string.IsNullOrEmpty(step))
                {
                    float sy = stepY - (s * 18f);
                    Text stepTxt = UIFactory.CreateText(reportRT, "Step" + s, step, new Vector2(-10, sy), 12, TextAnchor.MiddleLeft);
                    stepTxt.color = new Color(0.2f, 0.2f, 0.25f);
                    stepTxt.fontStyle = FontStyle.Normal;
                    stepTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 18);
                    Shadow sShadow = stepTxt.GetComponent<Shadow>();
                    if (sShadow != null) sShadow.effectColor = Color.clear;
                }
            }

            // Common mistake
            float mistakeY = stepY - 75f;
            string mistakeLabel = loc != null ? loc.Get("debrief_mistake_label") : "Vermeide diesen Fehler:";
            Text mistakeLabelTxt = UIFactory.CreateText(reportRT, "MistakeLabel", "❌ " + mistakeLabel, new Vector2(-15, mistakeY), 13, TextAnchor.MiddleLeft);
            mistakeLabelTxt.color = new Color(0.7f, 0.15f, 0.15f);
            mistakeLabelTxt.fontStyle = FontStyle.Bold;
            mistakeLabelTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 20);

            string dontText = loc != null ? loc.Get(debriefKey + "_hb_dont") : "";
            Text dontTxt = UIFactory.CreateText(reportRT, "DontText", dontText, new Vector2(-10, mistakeY - 18f), 12, TextAnchor.MiddleLeft);
            dontTxt.color = new Color(0.5f, 0.15f, 0.15f);
            dontTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 18);
            Shadow dontShadow = dontTxt.GetComponent<Shadow>();
            if (dontShadow != null) dontShadow.effectColor = Color.clear;

            // Key fact
            float factY = mistakeY - 45f;
            string factLabel = loc != null ? loc.Get("debrief_fact_label") : "Medizinischer Fakt:";
            Text factLabelTxt = UIFactory.CreateText(reportRT, "FactLabel", "💡 " + factLabel, new Vector2(-15, factY), 13, TextAnchor.MiddleLeft);
            factLabelTxt.color = new Color(0.15f, 0.4f, 0.7f);
            factLabelTxt.fontStyle = FontStyle.Bold;
            factLabelTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 20);

            string factText = loc != null ? loc.Get(debriefKey + "_hb_fact") : "";
            Text factTxt = UIFactory.CreateText(reportRT, "FactText", factText, new Vector2(-10, factY - 18f), 12, TextAnchor.MiddleLeft);
            factTxt.color = new Color(0.15f, 0.35f, 0.6f);
            factTxt.GetComponent<RectTransform>().sizeDelta = new Vector2(700, 18);
            Shadow factShadow = factTxt.GetComponent<Shadow>();
            if (factShadow != null) factShadow.effectColor = Color.clear;
        }

        // ── Record learning progress ──
        if (LearningTracker.Instance != null && success)
        {
            LearningTracker.Instance.RecordScenarioCompletion(missionName, accPercent);
        }

        // ── OK Button (premium styled) ──
        Color okBtnColor = success ? new Color(0.2f, 0.65f, 0.35f) : new Color(0.3f, 0.3f, 0.35f);
        GameObject okBtnObj = UIFactory.CreateButton(reportRT, "OKButton", "WEITER  →", new Vector2(0, -370), new Vector2(260, 50), winButtonSprite);
        Button okBtn = okBtnObj.GetComponent<Button>();
        // Style the button text
        Text okTxt = okBtnObj.GetComponentInChildren<Text>();
        if (okTxt != null) { okTxt.fontSize = 22; okTxt.color = Color.black; }
        ...
        okBtn.onClick.AddListener(() => {
            ...
            if (loadedFromMenu)
            {
                ...
                // Keep the player at their current coordinates instead of resetting the scene
            }
        });
    }
    ...
    /// <summary>
    /// Maps a mission name string to the corresponding localization key prefix (m1, m2, etc.)
    /// for the educational debrief section in ShowMissionReport.
    /// </summary>
    private string GetDebriefKey(string missionName)
    {
        ...
        // Fallback: try to match VictimType enum names
        if (lower.Contains("bikeaccident")) return "m1";
        if (lower.Contains("bleedingwound")) return "m2";
        if (lower.Contains("unconsciousperson")) return "m3";
        if (lower.Contains("burninjury")) return "m4";
        if (lower.Contains("heatstroke")) return "m6";
        if (lower.Contains("triagescene")) return "m9";
        if (lower.Contains("bonefracture")) return "m10";
        if (lower.Contains("allergicshock")) return "m11";
        if (lower.Contains("drowningvictim")) return "m12";
        if (lower.Contains("diabeticshock")) return "m13";
        if (lower.Contains("panicattack")) return "m14";
        return null;
    }
}
